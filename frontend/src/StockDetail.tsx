...
export function StockDetail({ ticker, onBack }: StockDetailProps) {
  ...
  return <section className="view-stack">
    ...
    <article className="panel"><div className="panel-heading"><div><span className="section-kicker">CATALYSTS</span><h2>Positive observations</h2></div><span className="source-tag">Fact-grounded</span></div><div className="insight-list">{stock.catalysts.length ? stock.catalysts.map((catalyst) => <div className="insight-row" key={catalyst.name}><div><strong>{catalyst.name}</strong><small>{catalyst.summary}</small></div></div>) : <div className="empty-state">No structured catalyst observations available.</div>}</div></article>
